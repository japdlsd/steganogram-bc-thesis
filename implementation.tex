...
Our goal was to separate the interface and internal logic. We've achieved that
by creating ``junction'' class \texttt{Solution}, which has been inserted between
Android-specific interface code and steganographic core, so that code, responsible for
...
Project (mainly the correct implementation of encoding/decoding images into and from the JPEG format). 
...
You can read more about Base64 encoding here \cite{josefsson2006base16}.
...
to test their mutual reversibility exhaustively (i.e. on every possible input).
...
the error thanks to a decryption of the inverse process in the paper. In this 
...
(this string should be parsable by function \texttt{loadKeyFromString}).
...
representing quantized DCT coefficients of a cover image with embedded message
